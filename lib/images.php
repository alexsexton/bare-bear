<?php
 ...
 // Removes height & widths on images
	function _bare_remove_thumbnail_dimensions( $html ) {
		$html = preg_replace( '/(width|height)=\"\d*\"\s/', "", $html);
		return $html;
	}
	add_filter( 'post_thumbnail_html', '_bare_remove_thumbnail_dimensions', 10 );
	add_filter( 'image_send_to_editor', '_bare_remove_thumbnail_dimensions', 10 );
	add_filter( 'the_content', '_bare_remove_thumbnail_dimensions', 10 );

  // Removes P Tags wrapped around images from WYSIWYG editor
	function _bare_filter_ptags_on_images($content) {
		$content = preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
		return preg_replace('/<p>\s*(<iframe .*>*.<\/iframe>)\s*<\/p>/iU', '\1', $content);
	}
	add_filter( 'acf_the_content', '_bare_filter_ptags_on_images' );
	add_filter( 'the_content', '_bare_filter_ptags_on_images' );
